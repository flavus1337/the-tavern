...
router.post('/:id/generate/save', bigJson, requireMember('dm'), async (req: Request, res: Response) => {
  ...
  try {
    // Props keep alpha (transparent cut-out); backgrounds are opaque maps.
    const img = sharp(buf).resize({ width: MAX_DIMENSION, height: MAX_DIMENSION, fit: 'inside', withoutEnlargement: true });
